...
export default function AccountPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            supabase
                .from('lost_items')
                .select('id, category, what_lost, lost_location, date_lost')
                .eq('user_id', user.id),
            supabase
                .from('found_items')
                .select('id, category, what_found, where_found, date_found')
                .eq('user_id', user.id),
    ...
    const reports = useMemo(() => {
        if (activeTab === 'lost') {
            return lostItems.map((row) => ({
                id: row.id,
                title: formatReportItemTitle(row.category, row.what_lost),
        ...
        return foundItems.map((row) => ({
            id: row.id,
            title: formatReportItemTitle(row.category, row.what_found),
